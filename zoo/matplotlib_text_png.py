...
from baseobject import baseobject as configuration

class text_png (configuration):

    def __init__ (self):
        super().__init__()

        self.fontpath = os.path.join(self.root, self.FONT_CHINESE_PATH)
        self.font = FontProperties(fname=self.fontpath, size=12)
    ...
    def writelines(self, strlist, filneame=None):
        ...
        #
        if filneame is None:
            filename = os.path.join(self.download, f'ctao3.text_png.png' )
